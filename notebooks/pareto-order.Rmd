...
We therefore treat the outcome of each objective as a random variable that follows some distribution $\mathcal{X}\sim\bm{\beta}$, where the parameters $\bm{\beta}$ are unknown.
...
For our purposes, the standard uniform distribution suffices, as its range is $\interval{0}{1}$, which corresponds to what one would expect from a score.
...
\begin{align}
  S_f:\mathbb{R}^m\mapsto\interval{0}{1}=1-\operatorname{CDF}_f(f(\mathbf{x}))\label{eq:score-for-f},
\end{align}

where the operation $\operatorname{CCDF}=1-\operatorname{CDF}$ is the _complementary_ $\operatorname{CDF}$.
...
When specifying preference, a DM might obtain a solution close to that preference. However, that solution is in the objective space $\mathcal{O}$, __not__ in the score space $\mathcal{S}$.
...
\begin{align}
  \mathcal{D}_k&=\left\{\left(\bm{\mathcal{P}}_i,\bm{\mathcal{S}}_i\right)\right\}_{i=1}^k\text{, dataset of bijections,}\label{eq:pref-bijections}
  \\[1ex]
...
The total scores for each of the solutions are in the interval $\interval{{\approx}1.848}{{\approx}2.856}$.
