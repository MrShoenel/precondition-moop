...
_Please note_: This is early-stage research. The canonical reference to the arXiv pre-print server is [@honel2022pareto].
...
We demonstrate an approach based on the marginal distributions of each objective to ordering single objectives, which further allows ordering Pareto efficient solutions.
Using no- or just few additional preferences for each objective, multi-objective optimization problems can effectively be transformed into single-objective optimization problems, which removes the requirement of a decision-maker.
...
Furthermore, by mapping objectives to scores, it allows a DM to gain insights into the density and homogeneity of the solution space, as well as to understand which trade-offs are actually possible.
...
The latter is perhaps the only space workable for human DMs. This means that so far, expressing preference with non-uniformly distributed objectives has most likely not led to the desired solution.
...
```{r ex2-high-prec, eval=TRUE, echo=FALSE, fig.cap="The empirical densities and cumulative distributions of the three objectives of the Viennet function.", fig.height=9, fig.show='hold', fig.align='center', out.width='90%'}
...
There is a unique best solution at $\mathbf{x}\approx\left\{-0.6708,0.4449\right\}$ for this problem with objective values of $\left\{f_1\approx0.9231,f_2\approx15.1532,f_3\approx0.0307\right\}$, marked by a red square in figure \ref{fig:ex2-pareto-front}. The solution marked by the purple square is the worst in the Pareto efficient set.
...
```{r ex2-tradeoffs-org-vs-corr, eval=FALSE, fig.cap="The absolute error of the original and corrected trade-offs.", fig.height=7, fig.align='center', out.width='90%'}
par(mfrow = c(2,2), mar = c(5,3,2,0))
temp0 <- abs(as.numeric(unlist(diffs_org[, paste0("DEVIATION_", 1:3)])))
temp1 <- abs(as.numeric(unlist(diffs_corr[, paste0("DEVIATION_", 1:3)])))

xld <- range(c(density(temp0)$x, density(temp1)$x))
xl <- range(c(temp0, temp1))

plot(density(temp0), xlim = xld, main = "Density abs. error original trade-offs.")
grid()
plot(ecdf(temp0), xlim = xl, main = "ECDF abs. error original trade-offs.")
grid()

plot(density(temp1), xlim = xld, main = "Density abs. error corrected trade-offs.")
grid()
plot(ecdf(temp1), xlim = xl, main = "Density abs. error corrected trade-offs.")
grid()
```
...
While still not perfect, $50$% of all trade-off ratios show a mean absolute error of ${\approx}0.07$.
We also obtain much less extreme deviations, with the $90$% quantile being ${\approx}0.46$ (compared to ${\approx}0.98$ of the original problem).
...
temp0 <- abs(as.numeric(unlist(diffs_org[, paste0("DEVIATION_", 1:3)])))
temp1 <- abs(as.numeric(unlist(diffs_corr[, paste0("DEVIATION_", 1:3)])))
temp2 <- abs(as.numeric(unlist(diffs_online[, paste0("DEVIATION_", 1:3)])))

temp <- `rownames<-`(x = rbind(
  paste0(seq(0, 100, by=10), "%"),
  round(quantile(x = temp0, probs = seq(0, 1, by=.1)), 6),
  round(quantile(x = temp1, probs = seq(0, 1, by=.1)), 6),
  round(quantile(x = temp2, probs = seq(0, 1, by=.1)), 6)), value = c("Quantile", "Original", "Corrected", "Online"))
...
if (interactive()) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    row.names = FALSE,
    caption = "Quantiles of trade-off absolute ratio errors for the original-, corrected- and online preferences.",
    label = "ex2-pref-corr-quant"
  )
}
```

```{r ex2-tradeoffs-org-vs-corr-online, eval=TRUE, fig.cap="The absolute error of the original, corrected, and online trade-offs.", fig.height=9, fig.align='center', out.width='90%'}
...
plot(density(temp0), xlim = xld, main = "Density abs. error original trade-offs.")
grid()
plot(ecdf(temp0), xlim = xl, main = "ECDF abs. error original trade-offs.", xlab = "Absolute ratio deviation")
grid()

plot(density(temp1), xlim = xld, main = "Density abs. error corrected trade-offs.")
grid()
plot(ecdf(temp1), xlim = xl, main = "ECDF abs. error corrected trade-offs.", xlab = "Absolute ratio deviation")
grid()

plot(density(temp2), xlim = xld, main = "Density abs. error online trade-offs.")
grid()
plot(ecdf(temp2), xlim = xl, main = "ECDF abs. error online trade-offs.", xlab = "Absolute ratio deviation")
grid()
```

```{r eval=FALSE}
c(mean(temp0), mean(temp1), mean(temp2))
```
...
Figure \ref{fig:ex2-tradeoffs-org-vs-corr-online} show a density of absolute errors for the online method similar to the one obtained using corrected  trade-offs. Approximately $50$% of each single ratio are off absolutely by $0.2$ or less for it.
For the method of corrected trade-offs, single approx. $50$% of all ratios are off by $0.07$ or less.
If we compare the quantiles of this method to the a priori correction method (table \ref{tab:ex2-pref-corr-quant}), then we see that the online method has a consistently larger error than the a priori correction method.
The mean absolute deviation for each ratio and method is ${\approx}0.5345$ (original), ${\approx}0.1545$ (corrected), and ${\approx}0.2915$ (online).
...
```{r}
temp <- ex2_pareto_front_scores[, c(paste0("s", 1:3), "s_total")]
#temp <- temp[temp$s1 > 0.7 & temp$s2 > 0.7 & temp$s3 > 0.7,]
temp$l1 <- 1 - temp$s1
temp$l2 <- 1 - temp$s2
temp$l3 <- 1 - temp$s3
temp$l_total <- 3 - temp$s_total

plot3D::scatter3D(x = temp$l1, y = temp$l2, z = temp$l3, colvar = temp$l_total, col = rev(viridis::viridis(100)), phi = 10, theta = 20, r = 2, ticktype = "detailed", nticks = 5, pch = 16, cex.p = 1, clab = "Loss", colkey = list("dist" = -0.1, "length" = .66, "width" = .66, "cex.clab" = 0.8), xlab = "Loss 1", ylab = "Loss 2", zlab = "Loss 3", bty = "b2", xlim = c(0,1), ylim = c(0,1), zlim = c(0,1))
```
```{r}
rgl::plot3d(x = temp$s1, y = temp$s2, z = temp$s3, xlab = "f1", ylab = "f2", zlab = "f3", xlim = c(0,1), ylim=c(0,1), zlim=c(0,1), add=FALSE)
```
...
ratio_to_pref <- function(r, normalize = FALSE) {
  s <- c(r[1], 1, 1/r[2])
  s <- s / max(s)
  if (normalize) {
    s <- s / sum(s)
...
```{r}
ratio_to_rel_len <- function(r) {
  # For example, 4:1 -> 1,4,4 / 9 = 1/9, 4/9, 4/9
  # 4 is the ratio between l1,l2, and 1 is the ratio between l2,l3
  # Or, in other words, if l1=l2, then l3 is one third of what l2 is
  # r is the ratios between interval pairs, not the intervals per se
  r <- r / max(r)
  lens <- c(1, rep(NA_real_, length(r)))
  for (i in 1:length(r)) {
    lens[i + 1] <- lens[i] * r[i]
  }
  lens / sum(lens)
...
rel_len_to_ratio <- function(l) {
  # For example, 2,3,4 = 2/9, 3/9, 4/9 -> 1.5:1.333
  ratios <- rep(NA_real_, length(l) - 1)
...
```{r eval=FALSE}
rgl::persp3d(x=ex2_tradeoff_density_2d$x, y = ex2_tradeoff_density_2d$y, z = ex2_tradeoff_density_2d$z,
...
```{r ex2-pstar-to-s, eval=TRUE, fig.cap="Mapping between feasible preferences and solutions in the score space.", fig.height=7, fig.align='center', out.width='90%'}
par(mfrow = c(2,2), mar = c(5,3,2,0))
temp <- ex2_possible_tradeoff_ratios_org_diffs$diffs_org_for_working
plot(density(temp$MAE), main = "Density MAE of feasible prefs.")
grid()
plot(density(temp$PERC_DIFF_1), main = "Percentual difference first score.")
grid()
plot(density(temp$PERC_DIFF_2), main = "Percentual difference second score.")
grid()
plot(density(temp$PERC_DIFF_3), main = "Percentual difference third score.")
grid()
```
