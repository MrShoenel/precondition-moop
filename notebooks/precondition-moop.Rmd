...
We will use the following surrogate functions to map the objectives of the optimization problem into a more numerically stable range.
In pre-conditioning, we want to learn a __bijective__ function that maps a loss to a score, so this is a valid thing to do.
...
In practice, one would perhaps be better of attempting to fit a wider range of potential distributions, then going with the best (using, e.g., the $D$-statistic of the KS-test).
...
The $\operatorname{ECCDF}$s of figure \ref{fig:ex1-eccdfs-norm} look quite similar to those we got empirically.
...
```{r ex1-working-tradeoffs, echo=FALSE, fig.cap="Working trade-offs using a pre-conditioned version of the first example problem."}
plot(x = c(), y = c(),
     xlab = latex2exp::TeX("$f_1(x)$"), ylab = latex2exp::TeX("$f_2(x)$"),
