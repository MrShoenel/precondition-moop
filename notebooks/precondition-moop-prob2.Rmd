...
The most important notion of what we are doing here, actually, is _orderability_ of the results. From that principle, we can derive a number of applications, such as correcting the weights a priori, or using the CDFs during minimization to get to some desired trade-off.
The solutions in the Pareto optimal set are, from a mathematical point of view, equal. More precisely, they are equal in Pareto's sense, that none of the solutions is dominated, and none of the solutions can be further improved.


However, using scores, we can add an _unsupervised_ way of introducing absolute order into the Pareto optimal set.
Scores could further be weighted to modify this ranking (each set of weights will yield one type of order).
...
for (i in 1:nrow(ex2_pareto_front_order)) {
  ex2_pareto_front_order[i,] <- c(
    ex2_high_prec_ecdfs$ecdf_funcs$ecdf_f1(ex2_pareto_front_original[i, 4]),
    ex2_high_prec_ecdfs$ecdf_funcs$ecdf_f2(ex2_pareto_front_original[i, 5]),
    ex2_high_prec_ecdfs$ecdf_funcs$ecdf_f3(ex2_pareto_front_original[i, 6]),
    0)
  ex2_pareto_front_order[i, 4] <- sum(ex2_pareto_front_order[i, 1:3])
}
```

As we will show, not all solutions are equally good using an ordering scheme where all objectives are equally important:

```{r}
range(ex2_pareto_front_order[, 4])
```


```{r}
plot(density(ex2_pareto_front_order[, 4]))
```
...
temp$col <- "black"
temp[temp$s_total > 1,]$col <- "blue"
temp[which.max(temp$s_total),]$col <- "red"
```


```{r}
rgl::plot3d(x = temp[, 4], y = temp[, 5], z = temp[, 6], col = temp$col)
```
...
Let's try to establish a non-linear understanding between the desired and gotten trade-offs.
We have already simulated the Pareto front, and we can use those results to carve out some training data.
It is important, however, that we properly preprocess the data: The wanted trade-off needs to be normalized (i.e., the most important weight will always be equal to one), and the gotten trade-off needs also to be mapped into the score space before normalizing it.

The goal of this approach is to obtain a model that a DM can use to transform their preference into the objective space.
In this approach, we work with the original (i.e., non-scaled) problem. We also do not need ideal-, nadir-, or utopian-vectors.
...
We have $50,000$ training examples. Let's take a few and learn an ANN.
We will treat the desired and gotten trade-offs as probabilities and therefore use the cross entropy loss function.
...
#scaler_X <- StandardScaler$new()
#train_x_scaled <- scaler_X$fit_transform(train_x)
#scaler_Y <- StandardScaler$new()
...
```{r}
diffs_org <- ex2_tradeoff_metrics_all(to_wanted = valid_y, to_gotten = valid_x)
```

```{r}
diffs_corr <- ex2_tradeoff_metrics_all(to_wanted = valid_y, to_gotten = pred)
```
...
plot(density(diffs_org$MAE), xlim = xld)
grid()
plot(ecdf(diffs_org$MAE), xlim = xl)
grid()

plot(density(diffs_corr$MAE), xlim = xld)
grid()
plot(ecdf(diffs_corr$MAE), xlim = xl)
grid()
```

```{r}
`rownames<-`(x = rbind(
  quantile(x = diffs_org$MAE, probs = seq(0, 1, by=.1)),
  quantile(x = diffs_corr$MAE, probs = seq(0, 1, by=.1))), value = c("org", "corr"))
```


```{r}
plot(density(diffs_tf$MAE), xlim = xld)
grid()
plot(ecdf(diffs_tf$MAE), xlim = xl)
grid()
```
...
```{r}
ex2_ml_calc_error <- function(y_true, y_pred) {
  nc <- ncol(y_true)
  nr <- nrow(y_true)
  s <- 0
  for (i in 1:nrow(y_true)) {
    s <- s + 1 / nc * sum((y_true[i,] - y_pred[i,])^2)
  }
  sqrt(s / nr) # RMSE
}
```
...
Let's show the uncorrected error, and the error as predicted from the network:

```{r}
c(
  "uncorr" = ex2_ml_calc_error(valid_y, valid_x),
  "pred" = ex2_ml_calc_error(valid_y, pred))
```
...
```{r}
par(mfrow = c(1,2))
plot(density(as.numeric(temp[, 3:5]), na.rm = TRUE), xlim = c(0, 270))
